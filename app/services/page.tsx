...
export async function POST(req: Request) {
    ...
    try {
        ...
        if (!validMatches.includes(reply)) {
            return NextResponse.json({ redirect: '/services?match=' + encodeURIComponent('AI content rewriting engine') });
        }

        return NextResponse.json({ redirect: '/services?match=' + encodeURIComponent(reply) });
    } catch {
